...
-   Добавить структуру *Weight*:
    -   Поле (инициализируется через конструктор с параметрами):
        -   значение веса
    -   Методы:
        -   *int CompareTo(Weight other)* - метод должен вернуть -1, если вес текущего экземпляра меньше, чем вес экземпляра из параметра. 0 - если они равны. 1 - если вес текущего экземпляра больше.
...
-   Внести изменения в классы, чтобы приложение работало корректно.
...
[Мини-проект. После темы -> Перегрузка операторов в C#]:----------------------------------------------------------

{{< expand "Мини-проект. После темы -> Перегрузка операторов в C#" "..." >}}

#### В рамках этого проекта вы будете выполнять задачи после окончания темы. Результатом выполнения задач будет мини-приложение по управлению складом. Модификаторы доступа выбирайте на своё усмотрение, если это не прописано явно. Старайтесь выдавать минимальный доступ к членам ваших классов.

---

-   Добавить перегрузку операторов *+*, *-*, *==*, *!=*, *>*, *<*, *>=* и *<=* для структуры *Volume*. Перегрузка операторов *+*, *-* должна работать аналогично методам *Add* и *Diff*.

-   Заменить метод *string ConvertToString()* на переопределение метода *ToString* для структуры *Volume*. Логика работы метода не должна измениться.

-   Добавить переопределение методов *Equals* и *GetHashCode* для структуры *Volume*.

-   Удалить методы *static Volume Diff(Volume volume1, Volume volume2)*, *static Volume Add(Volume volume1, Volume volume2)* и *int CompareTo(Volume other)* в структуре *Volume* или сделать их *private*.

-   Добавить перегрузку операторов *+*, *-*, *==*, *!=*, *>*, *<*, *>=* и *<=* для структуры *Weight*. Перегрузка операторов *+*, *-* должна работать аналогично методам *Add* и *Diff*.

-   Заменить метод *string ConvertToString()* на переопределение метода *ToString* для структуры *Weight*. Логика работы метода не должна измениться.

-   Добавить переопределение методов *Equals* и *GetHashCode* для структуры *Weight*.

-   Удалить методы *static Weight Diff(Weight weight1, Weight weight2)*, *static Weight Add(Weight weight1, Weight weight2)* и *int CompareTo(Weight other)* в структуре *Weight* или сделать их *private*.

-   Заменить метод *string ConvertToString()* на переопределение метода *ToString* для структуры *Price*. Логика работы метода не должна измениться.

-   Заменить метод *string ConvertToString()* на переопределение метода *ToString* для класса *Item* и его наследников. Логика работы метода не должна измениться.

-   Добавить переопределение методов *Equals* и *GetHashCode* для класса *Item*. Будем считать, что два товара эквивалентны, если их серийные номера совпадают (серийный номер будем считать регистронезависимым).

-   Добавить перечисление *CanAddItemResult* в проект типа *Class Library* со значениями *CanAdd*, *NoSpace* и *AlreadyExist*.

-   Изменить сигнатуру метода *bool CanAddItem(Item item)* на *CanAddItemResult CanAddItem(Item item)* в интерфейсе *IWarehouseManager*.

-   Изменить поле типа *List< Item >* на *HashSet< Item >* в классе *WarehouseManager*.

-   Добавить в метод *void AddItem(Item item)* класса *WarehouseManager* проверку, если текущий товар существует, то бросить *InvalidOperationException* с сообщением ```The item already exist```.

-   Изменить сигнатуру метода *bool CanAddItem(Item item)* на *CanAddItemResult CanAddItem(Item item)* в интерфейсе *WarehouseManager*. Если товар уже существует, нужно вернуть *AlreadyExist*. Если нет места - *NoSpace*. Если можно добавить товар - *CanAdd*.

-   Добавить проверку в метод *Execute* класса *AddItemCommand*. Если товар нельзя добавить, потому что он уже существует, отобразить сообщение ```The item already exist```.

---

{{< /expand >}}

[Мини-проект. После темы -> Анонимные и динамические типы. LINQ.]:----------------------------------------------------------

{{< expand "Мини-проект. После темы -> Анонимные и динамические типы. LINQ." "..." >}}
...
-   Удалить делегат *ItemPredicate*.

-   Изменить сигнатуру метода *ReadOnlyCollection< Item > GetItemsByCondition(ItemPredicate condition)* на *ReadOnlyCollection< Item > GetItemsByCondition(Func< Item, bool > condition)* в интерфейсе *IWarehouseManager*.

-   Изменить сигнатуру метода *ReadOnlyCollection< Item > GetItemsByCondition(ItemPredicate condition)* на *ReadOnlyCollection< Item > GetItemsByCondition(Func< Item, bool > condition)* в интерфейсе *WarehouseManager*. Изменить логику метода, чтобы он использовал *LINQ*.

-   Добавить перегрузку оператора *+* для структуры *Price*. Наследовать структуру от интерфейса *IComparable< Price >*.

-   Создать класс *Report*. Создать вложенный класс *ItemTypeInformation*:
    -   Конструктор *ItemTypeInformation(string type, IEnumerable< Item > items)*
    -   Свойства только для чтения:
        -   тип товара (инициализируется через конструктор с параметрами)
        -   товары, которые относятся к данному типу (инициализируются через конструктор с параметрами) - свойство должно содержать отсортированные по цене товары
        -   количество товаров данного типа (вычислять в конструкторе)
        -   сумма всех цен товаров данного типа (вычислять в конструкторе)


-   Добавить в класс *Report* следующие члены:
    -   Конструктор *private Report(IEnumerable< ItemTypeInformation > itemTypeInformation)*
    -   Свойства только для чтения:
        -   *IEnumerable< ItemTypeInformation >* (инициализируется через приватный конструктор с параметрами)
        -   общее количество товаров (вычислять в конструкторе)
        -   общая сумма цен всех товаров (вычислять в конструкторе)

-   Добавить метод *static Report Create(IEnumerable< Item > items)* в класс *Report*. Метод создаёт отчёт, который группирует товары по типу, а также содержит общее количество товаров и общую сумму цен всех товаров. Внутри каждой группы товары отсортированы по цене и для каждой такой группы отчёт содержит количество товаров и сумму цен.

-   Добавить метод *Report CreateReport()* в интерфейс *IWarehouseManager*.
...
-   Добавить класс *ShowReportCommand*, который наследуется от интерфейса *ICommand*:
    - Поле (инициализируется через конструктор с параметрами):
        -   поле типа *IWarehouseManager*
    - Метод:
        -   *void Execute()* - выполняет логику для команды ```Show report```. Если товаров нет, метод должен отобразить сообщение ```No items```, иначе - отобразить отчёт в формате:
```
{тип_товара_1}
------------------------------
{информация_о_товаре1_типа1}
...
{информация_о_товареN_типа1}
------------------------------
Count: {количество_товаров_типа_1}, Sum: {сумма_цен_товаров_типа_1}
------------------------------
...

{тип_товара_M}
------------------------------
{информация_о_товаре1_типаM}
...
{информация_о_товареN_типаM}
------------------------------
Count: {количество_товаров_типа_M}, Sum: {сумма_цен_товаров_типа_M}
------------------------------
Total count: {общее_количество_товаров}, Total sum: {общая_сумма_цен_всех_товаров}
```
